...
def checkout_finalize_view(request):
    ...
    if None in [sub_obj, user_obj, _user_sub_obj]:
        return HttpResponseBadRequest('There is an error with your subscription')
    if _user_sub_exists:
        # cancel old
        ...
        if old_stripe_id is not None and not same_stripe_id:
            try:
                helpers.billing.cancel_subscription(old_stripe_id, reason = "Auto ended.", feedback="other")
            except:
                pass
        # assign new sub
        ...
        _user_sub_obj.save()
        messages.success(request, 'Success! Thanks for subscribing.')
