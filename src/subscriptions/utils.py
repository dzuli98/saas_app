...
def sync_subs_group_permissions():
    ...
    for subs in qs:
        sub_perms = subs.permissions.all()
        for group in subs.groups.all():
...
def refresh_active_users_subscriptions(
    ...
    if day_start > 0 and day_end > 0:
        qs = qs.by_days_range(day_start, day_end)
    complete_count = 0
    qs_count = qs.count()
    
    for obj in qs:
        if verbose:
            print('Updating user', obj.user, 'subscription', obj.subscription_id, 'period end', obj.current_period_end)
        if obj.stripe_id:
